        ...
        if interface_type != "Port-channel-subinterface" and nso_to_oc_interface_types.get(interface_type):
            ...
            for nso_index, value in enumerate(config_before["tailf-ned-cisco-ios:interface"][interface_type]):
                ...
                if (nso_old_physical_interface_number == physical_interface_number) and (
                    ...
                    # The OC interface index is for physical interfaces, if sub-if remove the past increase
...
def xe_configure_ipv4_interface(nso_before_interface: dict, nso_leftover_interface: dict,
    ...
    if (nso_before_interface.get("ip") and not nso_before_interface.get("ip", {}).get("no-address")) or \
        ...
        if nso_before_interface.get("ip", {}).get("tcp", {}).get("adjust-mss"):
            ...
            if not nso_leftover_interface["ip"]["tcp"]:
                del nso_leftover_interface["ip"]["tcp"]
        ...
        if nso_before_interface.get("ip", {}).get("nat", {}).get("inside"):
            ...
            if not nso_leftover_interface["ip"]["nat"]:
                del nso_leftover_interface["ip"]["nat"]
    ...
    elif nso_before_interface.get("ip") and nso_before_interface.get("ip").get("no-address"):
        del nso_leftover_interface["ip"]["no-address"]

    if not nso_leftover_interface["ip"]:
        del nso_leftover_interface["ip"]
...
def xe_interfaces(config_before: dict, config_leftover: dict, interfaces: dict) -> None:
    ...
    for interface_type in interfaces:
        leftover_interfaces = []
        for interface in config_leftover["tailf-ned-cisco-ios:interface"][interface_type]:
            if len(interface) > 1:
                leftover_interfaces.append(interface)
        if not leftover_interfaces:
            del config_leftover["tailf-ned-cisco-ios:interface"][interface_type]
        else:
